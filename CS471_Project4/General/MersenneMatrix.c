 ...
 *  \brief Implementation of the create matrix method defined in General/MersenneMatrix.h
 *
 *  In this file the matrix of randomized values with numVectors x dimToTest dimensions is allocated
 *  and created using the Mersenne Twister algorithm referenced in General/m19937ar-cok.h.
 */
#include "Utilities.h"
...
double** createMatrix(EquationInfo info){
    ...
    minVal = info.range[RANGE_MIN_POS], /*! < Store the range minimum from the read in file for this equation locally for later calculations. RANGE_MIN_POS is defined in General/Utilities.h*/
    maxVal = info.range[RANGE_MAX_POS]; /*! < Store the range maximum from the read in file for this equation locally for later calculations. RANGE_MAX_POS is defined in General/Utilities.h*/
    /*!
...
double** createVelocities(EquationInfo info){
    int numVec = info.numVectors, /*! < Storing the value for number of vectors to test locally*/
            dim = info.dimToTest; /*! < Storing the value for the number of dimensions in the vector locally*/
    double minVal = info.range[RANGE_MIN_POS], /*! < Store the range minimum from the read in file for this equation locally for later calculations. RANGE_MIN_POS is defined in General/Utilities.h*/
    maxVal = info.range[RANGE_MAX_POS], /*! < Store the range maximum from the read in file for this equation locally for later calculations. RANGE_MAX_POS is defined in General/Utilities.h*/
