...
int checkTestType(){
    ...
    while(testFlag == Reading){
         ...
         * defined in the TestType enum within src/Utilities.h, otherwise if the user inputs 'Firefly' return Firefly
...
int processEquations(char* arg, Info* progInfo){
    ...
    /*!
     * if the number of equations defined is larger than the MAX_NUM_EQUATIONS defined in src/Utilities.h tell
...
int processIterations(char* arg, Info* progInfo){
     ...
     * greater than or equal to MINIMUM_ITERATIONS defined in src/Utilities.h tell the user
     * and return failure. Otherwise return success.
     */
...
int init(char* filename, Info* progInfo){
    ...
    char line[LINE_LENGTH]; /*!< Declare a char array of LINE_LENGTH defined in src/Utilities.h to store the current line being processed*/
